...
```bash
go test ./internal/ ./core/ ./cli/             # unit tests (offline); also `make test`
...
If the test binary or `bin/sg` aborts on macOS with `dyld: missing LC_UUID load command`, the local Go toolchain is too old for the OS: run with `-ldflags=-linkmode=external` or a newer toolchain (`GOTOOLCHAIN=go1.25.0 go test ...`).
...
- **`cli/` + `cmd/sg/`** — the `sg` command-line interface (cobra). `cmd/sg/main.go` just calls `cli.Execute()`. In `cli/`: `root.go` holds the global flags, env resolution (`SG_API_TOKEN`, `SG_BASE_URL`, `SG_ORG`), client construction and the `a.command(...)` leaf-command helper; `io.go` reads JSON bodies (`--body`/`--body-file`) and prints responses; `transport.go` is an `http.RoundTripper` that captures the raw response body so output is the API's JSON verbatim (re-marshaling the typed structs would drop extra properties); `flags.go` has the `flagString`/`flagInt`/`flagBool` helpers that only send a query parameter when its flag was set. One file per resource group (`workflows.go`, `stacks.go`, ...), each a method on `*app` returning a `*cobra.Command`.
...
- **CLI ↔ SDK parity**: every SDK method has exactly one CLI leaf command. `cli/tree_test.go` pins the full list (226 leaves); when adding an SDK method, add the command, a case in the matching `cli/*_test.go` table (the tests run against an `httptest` server and assert method, path, query, body and headers), and the new leaf in `expectedLeaves`.
- **Live-validated payload rules** (checked against a real organization on 2026-09-03): secret and role PATCH bodies must include `ResourceName` (the CLI fills it from the positional argument); workflow create for TERRAFORM/OPENTOFU needs `TerraformConfig`; state backend create needs `Statefiles` (an empty list is fine) and, for `aws_s3`, `StateBackendConfig.auth.integrationId`; resource search needs a non-empty `ResourceTypes`; template artifact endpoints take `NAME:REVISION`; several list endpoints answer 204 with no body (all `List*` methods set `ResponseIsOptional`). `tests/live/` re-checks that every read response decodes into the SDK types with no undeclared fields — run it after touching response types.
- **CLI request bodies** are decoded straight into the SDK request types. Unknown top-level keys are rejected (the SDK would drop them silently). A top-level JSON `null` on a `*core.Optional[T]` field is sent as an explicit null — `cli/io.go`'s `applyExplicitNulls` restores this because encoding/json nils a pointer field on `null` without calling `UnmarshalJSON`; nested Optional fields do not get this treatment. Union types (`PolymorphicPolicy`, `CreateTemplateRequest`) need their discriminator (`PolicyType`, `TemplateType`) in the JSON.
